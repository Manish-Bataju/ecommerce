...
export default function DesktopNavBar() {
  ...
  return (
    <div className='bg-navy text-white py-1.5 px-[clamp(0.5rem,3vw,3rem)] text-xl font-heading flex justify-between items-center shadow-[0_0_15px_rgba(0,0,0,0.2)]'>
      ...
      {/* LEFT SIDE: Logo */}
      <NavLink to='/'>
        <img src={logo} alt="Pukucha Logo" className="h-12 w-auto" />
      </NavLink>
        ...
        {categories.map((cat) => (
          <li key={cat.path}>
            <div className='flex flex-col items-center justify-center gap-0.5'>
              ...
              <span className='text-[.9rem] text-amber-200'>({cat.age})</span>
            </div>
          </li>
        ))}

        <li className='tracking-wide flex items-center'>
          <NavLink 
            to="/clearance" 
            className={({ isActive }) => 
              isActive 
                ? 'inline-block transition-transform duration-200 text-amber-200 text-xl font-semibold border-b-2 border-amber-200' 
